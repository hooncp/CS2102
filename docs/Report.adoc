...
This is facilitated by a helper function where we first find out their detailed schedule using `checkWorkingStatusHelperOfRider` function that joins the schedule table and interval table: (see Annex 10.1)
...
.Restaurant Staff can view and update their menu dynamically
...
BEGIN
    ...
    CASE
        ...
        WHEN test < '19:00:00' AND test > '17:00:00' then amount = 10.00;
        else amount = 5.00;
        END CASE;
    RETURN amount;
END;
